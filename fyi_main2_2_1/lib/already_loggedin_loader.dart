...
class AlreadyLoggedLoaderScreen extends StatefulWidget {
  @override
  _AlreadyLoggedLoaderScreenState createState() =>
      _AlreadyLoggedLoaderScreenState();
}
...
class _AlreadyLoggedLoaderScreenState extends State<AlreadyLoggedLoaderScreen> {
  Future<void> gethttp(String userloginname) async {
    ...
    try {
      ...
      finalexp = tempexperience
          .map<login.ExperienceList>(
              (json) => login.ExperienceList.fromJson(json))
          .toList();
      finaledu = tempedu
          .map<login.EducationList>(
              (json) => login.EducationList.fromJson(json))
          .toList();
      ...
      finalachievement = tempach
          .map<login.AchievementList>(
              (json) => login.AchievementList.fromJson(json))
          .toList();
      print(
          "${finalproject.length},${finaledu.length},${finalexp.length}, ${finalachievement.length}, ${finalskill.length}");
      ...
      for (int i = 0; i < finalskill.length; i++) {
        print("hello");
        skillexpandlist.add(SkillContainer(
            finalskill[i].field, finalskill[i].level.toString()));
      }
  ...
  Future Reload() async {
    ...
    if(route==1)
      ...
      {
        // await login.initiateFacebookLogin(context);
      }

    print(
        "HELOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOoo ${login.image} , ${login.finalproject}");
    if (login.image == null) {
      login.image =
    ...
    Navigator.push(
        ...
        MaterialPageRoute(
            builder: (context) => Testing(
                ...
                phone,
                finalskill,
                experienceexpandlist,
                skillexpandlist,
                educationexpandlist,
                achievementexpandlist,
                projectexpandlist,
                finalachievement,
                finalproject)));
  }
      ...
      body: Column(
        ...
        children: [
          ...
          Padding(
            ...
            child: Center(
                child: Text(
              "Please wait till we finish scribbling the changes onto your profile",
              style: TextStyle(
                fontSize: 18,
